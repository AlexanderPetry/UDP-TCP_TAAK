...
char* SRBytes(char *message,int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length ,int action);
int check_timeout(int sockfd, int timeout_sec, char* buf, struct sockaddr* addr, socklen_t* addrlen);
...
void execution( int internet_socket, struct sockaddr * internet_address, socklen_t internet_address_length )
{
	...
	SRBytes(input,internet_socket, internet_address, internet_address_length ,2);
	// SRBytes("",internet_socket, internet_address, internet_address_length ,1);

	while (1) {
    	// check if timeout has occurred
	    if (check_timeout(internet_socket, 3, temp_buffer, internet_address, &internet_address_length)) {
	...
	while (1) {
    	// check if timeout has occurred
	    if (check_timeout(internet_socket, 2, temp_buffer, internet_address, &internet_address_length)) {
	        printf("Timeout occurred\n");
	        char temp_val[1000];
			sprintf(temp_val,"%i",highest);
			SRBytes(temp_val,internet_socket, internet_address, internet_address_length ,2);  
	    }
	    highest = 0;
    	break;
	...
	if(strcmp(SRBytes("",internet_socket, internet_address, internet_address_length ,1), "OK") == 0)
	{
		printf("Now TCP");
	}
